...
RUN apt-get update && apt install -y apt-transport-https && curl -sS https://dl.yarnpkg.com/debian/pubkey.gpg | apt-key add - && \
  ...
  apt-get install -y apt-file apt-utils && apt-file update && apt-get install -y yarn vim less postgresql-client && \
  ...
  nvm install 6.11.5 && nvm alias default 6.11.5 && nvm use 6.11.5
